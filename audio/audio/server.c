...
int main(int argc,char *argv[])
{
    ...
    printf("the client ip is %s,port is %d\n",inet_ntop(AF_INET,&client.sin_addr,ip,sizeof(ip)),ntohs(port));
	
	run_cap = 1;
	ret = pthread_create(&pid_cap,NULL,Audio_Read,&connetfd);
	if(ret != 0)
	{
	...
	run_play = 1;
	ret = pthread_create(&pid_play,NULL,Audio_Play,&connetfd);
	if(ret != 0)
	{
		printf("pthread_create error!\n");
		return -1;
