...
void NTAG_Ins_Event(int fd,unsigned char *rBuf,unsigned char rlength)
{
	...
	CARD.SendLen = 0;	
	printf(">Rece = ");	
	uartSendHex(rBuf,rlength);	
	printf("\r\n");	
	
	ModifyReg(fd,JREG_COMMIEN,JBIT_RXI | JBIT_ERRI,RESET);//关闭接收中断	
	...
	if((ins == 0x50)&&(param == 0x00))//Halt指令
		{
			  ...
			  Card_Emulate_Init(fd);
				...
				ModifyReg(fd,JREG_MIFARE,BIT2,SET);
...
void NTAG_Event(int fd)
{
	...
	ModifyReg(fd,JREG_COMMIEN,JBIT_RXI | JBIT_HIALERTI,SET);						
