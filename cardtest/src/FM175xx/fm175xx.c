...
static unsigned char SetReg(int fd,unsigned char Reg, unsigned char value)
{
	...
	//printf("%s:%d set reg[%x] = %x \n",__FUNCTION__,__LINE__,Reg,value);
	addr = Reg & 0x7F;	
	uart_send(fd,addr);
	uart_read(fd);
	uart_send(fd,value);
	
	//printf("%s:%d get reg[%x] = %x \n",__FUNCTION__,__LINE__,Reg,GetReg(fd,Reg));
	return 0;
}
...
void pcd_RST(int fd)
{
	...
	if(ioctl(gpio_fd,GPIO_OUT_CMD,REST_PIN) < 0)
	{
                        ...
                        unsigned int *pOutLenBit,int flag)
{
   ...
   // Clear_BitMask(ComIrqReg, 0x80);
	...
	//printf("%s:%d FIFOLevelReg=%d\n",__FUNCTION__,__LINE__,Read_Reg(fd,FIFOLevelReg));
	if(flag)
	{
...
** Returned value:      TRUE：操作成功 ERROR：操作失败    
*********************************************************************************************************/
unsigned char FM175X_HardReset(int fd)
{    
    /*gpio_setValue(REST_PIN,0);
	usleep(1000);
	gpio_setValue(REST_PIN,1);
