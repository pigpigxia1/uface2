...
static unsigned char id_buff[64];
static int id_buff_len;
...
//		printf(">>>>>>>>>>111");
	...
	else if(machine_type == 2 )
	{
		...
		//
		usleep(50000);
		n = serial_read(fd,buff,20);
		
		printf(">>>>>>>>>>>>n = %d\n",n);
		if(n > 0)
		{
			//printf("n = %d\n",n);
			while(n < 20 && timeout < 10)
			{
				n = serial_read(fd,buff+num,20);
				num += n;
				usleep(50);
				timeout++;
				printf(">>>>>>>>>>>>n = %d\n",n);
