...
unsigned int crc_check(unsigned char* data, unsigned int length)
{
	...
	return (unsigned int)(~crc_reg) & 0x0000FFFF;
}
...
void * Pthread_Serial( void *arg )
{
    ...
    /*unsigned char Tx_Data[50]={0x55,0x55,0x07,0x03,0xB4,0xEE,0xEE}; */
    int ttyfd = *((int *)arg);
	printf("ttyfd = %d",ttyfd);
			...
			if (FD_ISSET(S_fd,&readfs))
			{
				...
				if(ret > 0)
				{
					...
					else
					{
						Rx_Data[ret] = '\0';
						printf("\%s\n",Rx_Data);
						if(strncmp(Rx_Data,"<CONNECTED>",strlen("<CONNECTED>")) == 0)
						{
							connect = 1;
							printf("connected\n");
						}
...
int uart_init(char *dev,int nSpeed,int nBits,char nEvent,int nStop)
{
    ...
    //Create a thread
	S_fd = fd;
    pthread_create( &pthread_id, NULL, &Pthread_Serial, &fd);
