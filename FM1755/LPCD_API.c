...
unsigned char Lpcd_Calibrate_Reference(void)
{
	...
	if(Lpcd_Debug_Info)
	{
		...
		printf("%x",i); 
		printf("\r\n");
		printf("; Lpcd.Calibration.Value = "); 
		printf("%x",Lpcd.Calibration.Value);
		printf("\r\n");
	}
    if((Lpcd.Calibration.Value == 0)&&(Lpcd.Calibration.Reference != 0))
      {
		printf("Calibra Reference Success!\r\n");	
        result = SUCCESS;
        break;//
      }
    if((Lpcd.Calibration.Value == 0)&&(Lpcd.Calibration.Reference == 0))
      {
        printf("Calibra Reference Error!\r\n");	
        result = ERROR; //
        break;
      }
    }
		...
		for(i = 0;i < 7;i ++ )//
        {
			...
			if((Lpcd.Calibration.Value > Lpcd.Calibration.Range_L)&&(Lpcd.Calibration.Value < Lpcd.Calibration.Range_H))
			{
             ...
             
				{
					...
					printf("%x",Lpcd.Calibration.Value); 
					printf("\r\n");//
					printf("; Lpcd.Fullscale_Value = "); 
					printf("%x",Lpcd.Fullscale_Value);
					printf("\r\n");
					printf("-> Driver = "); 
					printf("%x",Lpcd.Calibration.Driver_Index);
					printf("; Gain = ");  
					printf("%x",Lpcd.Calibration.Gain_Index); 
					printf("\r\n");
					printf("-> Lpcd.Calibration.Threshold_Max = "); 
					printf("%x",Lpcd.Calibration.Threshold_Max); //
					printf("; Lpcd.Calibration.Threshold_Min = "); 
					printf("%x",Lpcd.Calibration.Threshold_Min); 
					printf("\r\n");//*/
					...
					if(Lpcd_Debug_Test)
					{
						...
						{
							Lpcd_Get_Calibration_Value(&Lpcd.Calibration.Value);//
							printf("-> Lpcd.Calibration.Value2 = "); 
							printf("%x",Lpcd.Calibration.Value);
							printf("\r\n");
							//mDelay(500);
							usleep(100000);
...
void Lpcd_Set_Mode(unsigned char mode)
{
	...
	//printf("LPCD Mode Exited!\r\n");  
	}     
	//GetReg_Ext(JREG_LPCD_CTRL1,&reg);
	//printf("JREG_LPCD_CTRL1 reg=%d\n",reg);
    return;
}
...
unsigned char Lpcd_Set_IE(unsigned char mode)
{
    if(mode == 1)
        {
       	SetReg_Ext(JREG_LPCD_CTRL1,JBIT_BIT_CTRL_SET + JBIT_LPCD_IE + JBIT_LPCD_EN);//打开LPCD中断					
        }
    else
        {        
...
unsigned char Lpcd_Card_Event(void)
{
	...
	{		
		if(card_type & BIT0)
		{
			printf("-> TYPE_A Card Detected!\r\n");	
		/*	printf("-> ATQA = ");
			Uart_Send_Hex(Type_A.ATQA, 2);
			printf("\r\n");					
			printf("; UID = ");
			Uart_Send_Hex(Type_A.UID, 4 * Type_A.CASCADE_LEVEL);
			printf("\r\n");					
			printf("; SAK = ");
			Uart_Send_Hex(Type_A.SAK, Type_A.CASCADE_LEVEL);
			printf("\r\n");*/
		}							
		if(card_type & BIT1)
		{		
			printf("-> TYPE_B Card Detected!\r\n");
			/*printf("-> ATQB = ");Uart_Send_Hex(Type_B.ATQB,12);printf("\r\n");	
			printf("-> UID = ");
