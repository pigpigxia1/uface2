...
void Card_Emulate_Init(int fd)
{
   ...
   SetReg(fd,JREG_DIVIEN,JBIT_IRQPUSHPULL|JBIT_RFONI|JBIT_RFOFFI);
 	 Clear_BitMask(fd,JREG_CONTROL,JBIT_INITIATOR);		//清除Initiator位，进入卡模式
   return ;
}
...
unsigned char Card_Emulate_Config(int fd ,unsigned char *configbuf)
{
	...
	Clear_FIFO(fd);
  Write_FIFO(fd,25,configbuf);//SENS_RES(2 bytes),NFCID1(3 bytes),SEL_RES(1 byte),NFCID3(1 byte)

	SetReg(fd,JREG_COMMAND,CMD_CONFIGURE);
	usleep(1000);			//1ms
	reg_data = GetReg(fd,JREG_COMMAND);	
...
unsigned char Card_Emulate_IRQ_Event(int fd,unsigned char tag_com_irq,unsigned char tag_div_irq)
{
	unsigned char reg_com_irq,reg_div_irq;
		CARD.IRQ = FALSE;	
		reg_com_irq = GetReg(fd,JREG_COMMIRQ);		
		reg_div_irq = GetReg(fd,JREG_DIVIRQ);		
...
void Card_Emulate_RX(int fd,unsigned char *rece_buf,unsigned char *rece_len)
{
	...
	*rece_len = 0;
	...
	{	
		...
		{
			Read_FIFO(fd,32,&rece_buf[*rece_len]);		//渐满之后读32字节
					
			*rece_len += 32;
		}		
		regdata = GetReg(fd,JREG_COMMIRQ);	
	}
	while(!(regdata & JBIT_RXI));

	length = FM175XX_Get_FIFOlength(fd);	//

	Read_FIFO(fd,length,&rece_buf[*rece_len]);		//读最后的数据
...
unsigned char Card_Emulate_TX(int fd,unsigned char ilen,unsigned char *ibuf)
{
	...
	if(slen <= 64)
	{
		Write_FIFO(fd,slen,sbuf);		//write fifo	长度小于64字节，全部写入FIFO
		slen = 0;
	}
	else
	{
		Write_FIFO(fd,64,sbuf);			//write fifo	长度大于64字节，先写入64字节
	...
	{		
		if(FM175XX_Get_FIFOlength(fd)<=32)
		{
			if(slen<=32)
			{
				Write_FIFO(fd,slen,sbuf);			//FIFO写入剩余字节
				slen = 0;				
			}
			else
			{
				Write_FIFO(fd,32,sbuf);			//FIFO写入32字节
